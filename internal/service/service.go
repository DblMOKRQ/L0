...
func NewOrderService(consumer Consumer, repository OrderRepository, redisClient RedisClient, log *zap.Logger) *OrderService {
	return &OrderService{consumer: consumer, repository: repository, redisClient: redisClient, log: log.Named("OrderService")}
}
...
func (s *OrderService) SetOrder(ctx context.Context, order *models.Order, ttl time.Duration) error {
	key := fmt.Sprintf("order:%s", order.OrderUID)
	return s.redisClient.SetOrder(ctx, order, ttl, key)
}
